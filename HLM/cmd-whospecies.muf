...
Lists the people in the room, their genders, and species. The 'status' 
command can set your Status field; see 'status #help'. If someone's ws #far 
shows "redacted," they have used the #object option to hide that data. ws 
also has several customization options:
...
The first three options can also be turned off with, for example, 'ws 
#!object'.
 
Players are listed in order of their arrival in the room by default. #sort 
will order them by the given criterion. You can also '@set 
me=_prefs/ws/sort:<criterion>' to sort by <criterion> when no #sort is 
specified; use 'ws #sort none' to list in the normal order of arrival.
