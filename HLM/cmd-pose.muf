...
: main  ( str -- )
    ...
    dup address? if  ( strY strZ strX adr )
        ...
        "I don't recognize the command '#%s'. Try 'pose #help' for help, or using '##' to pose something starting with '#'." fmtstring tell  ( strY strZ )
        pop pop  (  )
    then  (  )
;
.
...
Emotes <message> to the room, like /me on IRC. That is, if Natasha
typed "pose waves!", she and everyone else in the room would see "Natasha
waves!" Use #ignore <name> to not see <name>'s says, poses, and spoofs.
Use #grey to not see color in others' says and poses.
 
Use #nickname to enable nickname posing, the use of your %N name instead of 
your character's actual name. You can set your %N nickname with "@set 
me=%N:<nickname>". The first three letters of your %N must match those of 
your real name, and must not be another player's name; that is, White-
Delirium can be "White" but not "Delimink" and not "Whistlewind" (assuming 
that name is taken by another player).
 
Use #!nicks to disable others' nickname posing.
.end
@action pose;pos;po;delimiter;/me=#0=tmp/exit1
